...
const directNavigation: NavigationItem[] = [{ key: "dashboard", href: "/dashboard", label: "Pulpit", glyph: "◫" }];
...
export default function AppWorkspaceShell({ account, portfolios, activePortfolioId, activeBaseCurrency, isPortfolioMutationPending, isLoggingOut, isAdmin, onPortfolioChange, onBaseCurrencyChange, onQuickAdd, onLogout, children }: AppWorkspaceShellProps) {
  ...
  return <div className={isSidebarCollapsed ? "workspace-shell is-sidebar-collapsed" : "workspace-shell"}>
    <aside className="workspace-sidebar" aria-label="Główna nawigacja">
      <div className="workspace-brand-row"><Link href="/dashboard" className="workspace-brand" aria-label="Mexo — pulpit"><span className="workspace-brand-mark" aria-hidden="true">M</span><span className="workspace-brand-name">Mexo</span></Link><button type="button" className="workspace-sidebar-toggle" onClick={() => setIsSidebarCollapsed((current) => !current)} aria-label={isSidebarCollapsed ? "Rozwiń menu" : "Zwiń menu"} title={isSidebarCollapsed ? "Rozwiń menu" : "Zwiń menu"}><span aria-hidden="true">‹</span></button></div>
      <nav className="workspace-sidebar-nav" aria-label="Sekcje aplikacji">
        {directNavigation.map((item) => <NavigationLink key={item.key} item={item} active={route === item.key} compact={isSidebarCollapsed} />)}
        {navigationGroups.map((group) => <details key={group.label} className="workspace-nav-group" open={isGroupActive(route, group)}><summary>{group.label}</summary><div className="workspace-nav-group-links">{group.items.map((item) => <NavigationLink key={item.key} item={item} active={route === item.key} compact={isSidebarCollapsed} />)}</div></details>)}
      </nav>
      <div className="workspace-sidebar-bottom">
        ...
        <NavigationLink item={{ key: "settings", href: "/settings", label: "Ustawienia", glyph: "⚙" }} active={route === "settings"} compact={isSidebarCollapsed} />
    ...
    <header className="workspace-mobile-header"><Link href="/dashboard" className="workspace-mobile-brand" aria-label="Mexo — pulpit"><span>M</span></Link><div><small>{meta.eyebrow}</small><strong>{meta.title}</strong></div><select value={activePortfolio?.id ?? ""} onChange={(event) => onPortfolioChange(event.target.value)} aria-label="Wybierz aktywny portfel" disabled={isPortfolioMutationPending}>{portfolios.map((portfolio) => <option key={portfolio.id} value={portfolio.id}>{portfolio.name}</option>)}</select></header>
    <div className="workspace-main-shell"><header className="workspace-topbar"><div className="workspace-topbar-copy"><span className="workspace-breadcrumb">{meta.breadcrumb}</span><h1>{meta.title}</h1></div><div className="workspace-topbar-actions"><label className="workspace-portfolio-select"><span>Aktywny portfel</span><select value={activePortfolio?.id ?? ""} onChange={(event) => onPortfolioChange(event.target.value)} disabled={isPortfolioMutationPending}>{portfolios.map((portfolio) => <option key={portfolio.id} value={portfolio.id}>{portfolio.name}</option>)}</select></label><label className="workspace-currency-select"><span>Waluta</span><select value={activeBaseCurrency} onChange={(event) => onBaseCurrencyChange(event.target.value)} disabled={isPortfolioMutationPending}><option value="PLN">PLN</option><option value="USD">USD</option><option value="EUR">EUR</option><option value="GBP">GBP</option></select></label><button type="button" className="workspace-topbar-add" onClick={onQuickAdd}>Dodaj aktywo</button><Link href="/settings" className="workspace-profile-link" aria-label="Ustawienia konta">{(account.email[0] ?? "M").toUpperCase()}</Link></div></header><main className="workspace-content" id="workspace-content" tabIndex={-1}>{children}</main></div>
    <nav className="workspace-bottom-nav" aria-label="Nawigacja mobilna"><NavigationLink item={directNavigation[0]} active={route === "dashboard"} /><NavigationLink item={{ key: "positions", href: "/portfolio/positions", label: "Portfel", glyph: "≡" }} active={isGroupActive(route, navigationGroups[0])} /><button ref={quickAddTriggerRef} type="button" className="workspace-mobile-add" onClick={() => setIsQuickAddOpen(true)} aria-label="Dodaj aktywo" aria-haspopup="dialog" aria-expanded={isQuickAddOpen}>+</button><NavigationLink item={{ key: "performance", href: "/analytics/performance", label: "Analiza", glyph: "↗" }} active={isGroupActive(route, navigationGroups[1])} /><button ref={moreTriggerRef} type="button" className={isMoreOpen ? "workspace-mobile-more is-active" : "workspace-mobile-more"} onClick={() => setIsMoreOpen(true)} aria-haspopup="dialog" aria-expanded={isMoreOpen}><span aria-hidden="true">•••</span><small>Więcej</small></button></nav>
    ...
    {isMoreOpen ? <div className="workspace-sheet-backdrop" role="presentation" onMouseDown={() => setIsMoreOpen(false)}><section ref={moreDialogRef} tabIndex={-1} className="workspace-mobile-sheet workspace-more-sheet" role="dialog" aria-modal="true" aria-label="Więcej sekcji" onMouseDown={(event) => event.stopPropagation()}><div className="workspace-sheet-handle" aria-hidden="true" /><p className="eyebrow">Mexo</p><h2>Więcej</h2><div className="workspace-sheet-links">{navigationGroups.flatMap((group) => group.items).filter((item) => !["positions", "performance"].includes(item.key)).map((item) => <Link key={item.key} href={item.href} onClick={() => setIsMoreOpen(false)}><span aria-hidden="true">{item.glyph}</span>{item.label}</Link>)}<Link href="/settings" onClick={() => setIsMoreOpen(false)}><span aria-hidden="true">⚙</span>Ustawienia</Link></div><button type="button" className="workspace-logout-link" onClick={onLogout} disabled={isLoggingOut}>{isLoggingOut ? "Wylogowywanie…" : "Wyloguj"}</button></section></div> : null}
  </div>;
}
